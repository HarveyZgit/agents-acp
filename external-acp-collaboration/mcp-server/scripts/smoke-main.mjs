...
async function waitFor(runId, expected) {
  for (let attempt = 0; attempt < 50; attempt += 1) {
    const status = await call("get_external_agent_status", { runId });
...
try {
  ...
  assert.ok(tools.tools.some((tool) => tool.name === "respond_external_agent_permission"));

  const providers = await call("list_external_agent_providers", {});
  assert.ok(providers.some((provider) => provider.provider === "fake" && provider.available));

  const first = await call("start_external_agent", {
    provider: "fake",
    cwd: workspace,
  ...
  await call("respond_external_agent_permission", {
    runId: first.id,
    requestId: "rpc-900",
    decision: "allow-once",
    userConfirmed: true,
  ...
  const completed = await call("get_external_agent_result", { runId: first.id });
  assert.match(completed.summary, /Fake ACP/);
  assert.deepEqual(completed.changedFiles, [{ path: "fake-output.txt", kind: "modify" }]);

  const resumed = await call("resume_external_agent", {
    runId: first.id,
  ...
  const cancellable = await call("start_external_agent", {
    provider: "fake",
    cwd: workspace,
  ...
  const cancelled = await call("cancel_external_agent", { runId: cancellable.id });
