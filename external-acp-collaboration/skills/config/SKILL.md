...
3. Report **every** provider's confirmed `launch` (`argv`, `source`, `spawn: "direct"`): Cursor (`cursor-agent`), Grok (`grok`), and Antigravity (`agy_acp_server.par`). Also report `setupQuestions` id `confirmedLaunch`. The plugin spawns that official file with `shell:false`. If `ignoredWrappers` lists a `cursor-agent` / `cursor` / `agent` / `grok` / `agy` / `antigravity` function, alias, or PATH file, tell the user it was detected and **will not be used**. Never invoke those wrapper names from the host shell to “help” setup — do not follow a user-modified CLI function (for example `cursor-agent`, `grok`, or `agy` checking the network first).
